        ...
        <Link href={`/auction/${auction.id}`}>
            <Card className="h-full flex flex-col">
                <div className="flex items-start justify-between mb-4 gap-2">
                    <h3 className="text-xl md:text-2xl">{auction.title}</h3>
                    <Badge status={statusMap[auction.status]}>
                        {auction.status}
                    </Badge>
                </div>

                <p className="text-muted font-mono text-sm md:text-base mb-4 md:mb-6 flex-grow line-clamp-2">
                    {auction.description}
                </p>
                ...
                <div className="space-y-3 md:space-y-4">
                    ...
                    <div className="flex justify-between items-center border-t-3 border-border pt-3 md:pt-4">
                        <span className="font-mono text-xs md:text-sm text-muted">BIDDERS</span>
                        <span className="font-mono text-base md:text-lg font-bold">{bidCount}</span>
                    </div>
                </div>
            </Card>
        </Link>
